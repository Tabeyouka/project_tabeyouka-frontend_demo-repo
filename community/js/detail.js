(() => {
    ...
    fetch("http://127.0.0.1:8080/api/status", {
      ...
      .then((data) => {
        ...
        commentInput.addEventListener("input", () => {
          commentData2.text = commentInput.value;
          test.text = commentInput.value;
        });
      })
  ...
  registerBtn.addEventListener("click", () => {
    ...
    // 입력받은 댓글 정보를 JSON 데이터로 만듦
    let jsonData = JSON.stringify(commentData2);
  ...
  const createComments = () => {
    ...
    commentData.forEach((comment) => {
      ...
      commentAuthorDate.textContent = `${comment.nickname} ${formatDate(
        comment.created_at
      )}`;
  ...
  const commentEdit = () => {
    ...
    comModifyBtn.forEach((button) => {
      button.addEventListener("click", (e) => {
        ...
        editId = buttonContainer.getAttribute("data-id1");
        editCommentData.author_id = buttonContainer.getAttribute("data-id2");
        checkUser.author_id = buttonContainer.getAttribute("data-id2"); 

        // 댓글을 입력한 사용자가 아닐 시 모달 창을 띄워 수정 권한이 없음을 표시
    ...
    // 수정하는 입력값을 editCommentData에 저장
    modifiedComment.addEventListener("input", () => {
      if (modifiedComment.value === "") {
        document.querySelector(".errorMsg").textContent =
          "수정할 내용을 입력해주세요.";
      }
      editCommentData.text = modifiedComment.value;
    });

    // 수정완료 버튼을 누르면 수정된 데이터를 서버로 POST 후 댓글을 다시 불러옴
    document.querySelector(".com-modify-btn2").addEventListener("click", () => {
      fetch(`http://localhost:8080/api/post/${postId}/comments/${editId}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const commentDel = () => {
    ...
    comDeleteBtn.forEach((button) => {
      button.addEventListener("click", (e) => {
        ...
        deleteId = buttonContainer2.getAttribute("data-id1");
        delCommentData.author_id = buttonContainer2.getAttribute("data-id2");
        checkUser.author_id = buttonContainer2.getAttribute("data-id2");

        console.log("dsf", delCommentData);

        // 댓글을 입력한 사용자가 아닐 시 모달 창을 띄워 삭제 권한이 없음을 표시
        if (checkUser.author_id != userInfo.id) {
          ...
          if (confirmation) {
            fetch(
              `http://127.0.0.1:8080/api/post/${postId}/comments/${deleteId}`,
              {
                method: "DELETE",
                headers: {
                  "Content-Type": "application/json ",
